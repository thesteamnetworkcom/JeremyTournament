		...
		<div className={classes.fullScreen}>
			...
			<div className={classes.vanguardWrapper}>
				<div className={classes.leftSide}>
					{Array.isArray(state.players) & Array.isArray(state.vanguards) !== false ? state.players.map((el, index) => index < 4 ?
				...
				<div className={classes.rightSide}>
					{Array.isArray(state.players) & Array.isArray(state.vanguards) !== false ? state.players.map((el, index) => index > 3 ?
						<Card className={classes.playerCard} key={index}>
							<span>{el.name}</span>
							<FormControl variant="outlined">
								<InputLabel id={el.name + "-Vanguard-Label"}>Vanguard</InputLabel>
